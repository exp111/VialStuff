...
#define PLUGIN_NAME "Vial Plugin"
...
const char* ts3plugin_description() {
	/* If you want to use wchar_t, see ts3plugin_name() on how to use */
    return "Changes vial rgb according to the mute status";
}
...
void ts3plugin_onUpdateClientEvent(uint64 serverConnectionHandlerID, anyID clientID, anyID invokerID, const char* invokerName, const char* invokerUniqueIdentifier)
{
	int result;
	anyID localID = 0;
	result = ts3Functions.getClientID(serverConnectionHandlerID, &localID);
	if (result != ERROR_ok)
	{
		ts3Functions.printMessageToCurrentTab("no clientid");
		return;
	}
	
	if (clientID != localID) // only check for the local user
		return;
